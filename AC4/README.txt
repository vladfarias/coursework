...
Cada médico é identificado pelo seu CRM, possui um CPF, um nome, um telefone e recebe um salário na clínica. 

Um médico pode ter formação em diversas especialidades (ortopedia, traumatologia, infectologia, etc), mas só exerce uma delas na clínica.
...
Desenvolver um programa orientado a objetos em Python que atenda as especificações descritas acima.

O programa deve especificar as classes, atributos e métodos necessárias para a solução do problema.

O programa deve aplicar os conceitos de encapsulamento, herança, polimorfismo (sobrescrita de métodos) e classe abstrata.

Além das classes, deve ser desenvolvido um programa principal, onde devem ser instanciados os objetos e demonstrar o correto 
funcionamento do sistema (por exemplo: associar um paciente a um quarto, inserir as anotações sobre o paciente em seu histórico, etc.)

Não é necessário realizar inputs. Os objetos podem ser definidos no próprio código fonte.
